...
export async function POST(request: NextRequest) {
  try {
    ...
    const existingBrand = await query("SELECT id FROM brands WHERE name = ?", [
      name,
    ]);
    ...
    // Create new brand
    const [result] = await execute("INSERT INTO brands (name) VALUES (?)", [
      name.trim(),
    ]);

    const insertResult = result as { insertId: number };
    return NextResponse.json(
      { id: insertResult.insertId, name: name.trim() },
      { status: 201 },
    );
  } catch (error) {
